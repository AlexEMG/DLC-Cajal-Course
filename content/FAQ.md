...
2) Blury images
   - If there is a lot of motion blur and you abosultely need to label blurry bodyparts, you should do it but try to be as consistent as possible
3) 'OOM when allocating tensor with shape'
   - You're trying to use more memory than is available on your GPU. Consider cropping unnecesarry parts of the image, changing the `batch_size` or the `global_scale`
     in pose_cfg.yaml
...
8) Video analysis stuck at a specific point/cannot start analysis 
   - Most likely this frame of the video was corrupted/the whole video has been corrupted. Reencode the video following the amazing guide (:grin:) written by @KonradDanielewski
